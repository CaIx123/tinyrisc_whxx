...
module uart_debug(
    ...
    // [MODIFIED] rom_wait_cnt �߼�
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            rom_wait_cnt <= 3'd0;
        end else if (state == S_WRITE_MEM) begin
            if (write_mem_byte_index0 < (need_to_rec_bytes + 2)) begin
                if (rom_wait_cnt < 3'd5) rom_wait_cnt <= rom_wait_cnt + 1'b1;
                else rom_wait_cnt <= 3'd0;
            end else rom_wait_cnt <= 3'd0;
        end else begin
            rom_wait_cnt <= 3'd0;
        end
    end
...
// [MODIFIED] ��״̬���߼�
    ...
    // [MODIFIED] ��д�̼���ַ�����ݵ����߼� (���� rom_wait_cnt == 5 �ж�)
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            write_mem_addr <= 32'h0; write_mem_data <= 32'h0;
            write_mem_byte_index0 <= 8'h0; write_mem_byte_index1 <= 8'h0;
            write_mem_byte_index2 <= 8'h0; write_mem_byte_index3 <= 8'h0;
        end else begin
            case (state)
                S_REC_FIRST_PACKET: begin write_mem_addr <= `ROM_START_ADDR; end
                S_CRC_END: begin
                    //if (write_mem_addr > 0) write_mem_addr <= write_mem_addr - 4;
                    write_mem_data <= {rx_data[4], rx_data[3], rx_data[2], rx_data[1]};
                    write_mem_byte_index0 <= 8'h5; write_mem_byte_index1 <= 8'h6;
                    write_mem_byte_index2 <= 8'h7; write_mem_byte_index3 <= 8'h8;
                end
                S_WRITE_MEM: begin
                    if (rom_wait_cnt == 3'd5) begin
...
// --- ���ݰ��������� ---
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_REC_FIRST_PACKET, S_REC_REMAIN_PACKET: begin
                    rec_bytes_index <= 8'h0;
                end
            endcase
        end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                S_REC_FIRST_PACKET:  need_to_rec_bytes <= `UART_FIRST_PACKET_LEN;
                S_REC_REMAIN_PACKET: need_to_rec_bytes <= `UART_REMAIN_PACKET_LEN;
            endcase
        end
    end
    ...
    always @ (posedge clk) begin
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            fw_file_size <= 32'h0;
        end else if (state == S_CRC_START) begin
        ...
        if (rst == 1'b0 || debug_en_i == 1'b0) begin
            ...
            case (state)
                ...
                S_CRC_CALC: begin
                    ...
                    if (crc_bit_index < 4'h9) crc_bit_index <= crc_bit_index + 1'b1;
                    else crc_bit_index <= 4'h0;
                    
                    if (crc_bit_index == 4'h0) crc_byte_index <= crc_byte_index + 1'b1;
                end
            endcase
        end
    end
