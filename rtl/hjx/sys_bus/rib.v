...
module rib #(
    ...
    parameter [SLAVE_NUM-1:0] SLAVE_MASK = {SLAVE_NUM{1'b1}})(
    ...
    for (i = 0; i < SLAVE_NUM; i = i + 1) begin: s_sel
        assign slave_sel[i] = SLAVE_MASK[i] & (mux_m_addr[31:28] == i);
    end
    ...
    reg txn_pending;
    reg[MASTER_NUM-1:0] master_sel_r;
    reg[SLAVE_NUM-1:0] slave_sel_r;
    wire[MASTER_NUM-1:0] master_rsp_sel = txn_pending ? master_sel_r : master_sel_vec;
    wire[SLAVE_NUM-1:0] slave_rsp_sel = txn_pending ? slave_sel_r : slave_sel;
    ...
    always @ (*) begin: s_out
        ...
        for (j = 0; j < SLAVE_NUM; j = j + 1) begin: s_sig_out
            mux_s_data    = mux_s_data    | ({32{slave_rsp_sel[j]}} & slave_data[j]);
            mux_s_req_rdy = mux_s_req_rdy | ({1 {slave_sel[j]}} & slave_req_rdy[j]);
            mux_s_rsp_vld = mux_s_rsp_vld | ({1 {slave_rsp_sel[j]}} & slave_rsp_vld[j]);
        end
    end

    reg mux_rsp_rdy;

    always @ (*) begin: rsp_rdy_out
        mux_rsp_rdy = 1'b0;
        for (j = 0; j < MASTER_NUM; j = j + 1) begin: rsp_rdy_sig_out
            mux_rsp_rdy = mux_rsp_rdy | ({1 {master_rsp_sel[j]}} & master_rsp_rdy[j]);
        end
    end

    wire req_hsked = mux_m_req_vld & mux_s_req_rdy & (~txn_pending);
    wire rsp_hsked = mux_s_rsp_vld & mux_rsp_rdy & txn_pending;

    always @ (posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            txn_pending <= 1'b0;
            master_sel_r <= {MASTER_NUM{1'b0}};
            slave_sel_r <= {SLAVE_NUM{1'b0}};
        end else if (req_hsked) begin
            txn_pending <= 1'b1;
            master_sel_r <= master_sel_vec;
            slave_sel_r <= slave_sel;
        end else if (rsp_hsked) begin
            txn_pending <= 1'b0;
        end
    end
    ...
    for (i = 0; i < MASTER_NUM; i = i + 1) begin: demux_m_sig
        assign demux_m_req_rdy[i]            = {1 {master_sel_vec[i]}} & mux_s_req_rdy & (~txn_pending);
        assign demux_m_rsp_vld[i]            = {1 {master_rsp_sel[i]}} & mux_s_rsp_vld;
        assign demux_m_data[(i+1)*32-1:32*i] = {32{master_rsp_sel[i]}} & mux_s_data;
    end
    ...
    for (i = 0; i < SLAVE_NUM; i = i + 1) begin: demux_s_sig
        ...
        assign demux_s_req_vld[i]            = {1 {slave_sel[i]}} & mux_m_req_vld & (~txn_pending);
        assign demux_s_rsp_rdy[i]            = {1 {slave_rsp_sel[i]}} & mux_rsp_rdy;
