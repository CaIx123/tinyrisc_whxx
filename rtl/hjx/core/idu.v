...
module idu(
    ...
    wire inst_sid = opcode_0101111 & funct3_000;
    wire inst_rt = opcode_0101111 & funct3_001;
    wire inst_if = opcode_0101111 & funct3_010;
    ...
    wire inst_sel_i_imm = inst_addi | inst_slti | inst_sltiu | inst_xori | inst_ori | inst_andi | inst_type_load | inst_jalr | inst_if;
    ...
    wire op_alu = inst_lui | inst_auipc | (inst_type_alu_imm & (~inst_nop)) | inst_type_alu_reg;
    wire op_bjp = inst_jal | inst_jalr | inst_type_branch;
    wire op_sys = inst_nop | inst_fence | inst_fence_i;
    ...
    // 是否需要访问rs2寄存器
    wire access_rs2 = inst_type_alu_reg | inst_type_store | inst_type_branch | inst_if;

    assign rs2_raddr_o = access_rs2? (inst_if? 5'h1f : rs2) : 5'h0;

    // 是否需要访问rd寄存器
    wire access_rd = op_alu | inst_jal | inst_jalr | inst_type_load | inst_rt | inst_if;
