...
module ifu(
    ...
    always @ (posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            discard_rsp <= ~ifu_rsp_hsked;
        end else if (discard_rsp & ifu_rsp_hsked) begin
            discard_rsp <= 1'b0;
        end
    end
    ...
    // 只有�??2种情况下取出来的指令是有效的，这里要把这两种情况识别出来
    wire bus_switched = req_switched & rsp_switched;

    // 取指地址
    assign ibus_addr_o = pc;
    assign pc_o = pc_r;
    wire inst_valid = ifu_rsp_hsked & (~flush_i) & (~bus_switched) & (~discard_rsp);
