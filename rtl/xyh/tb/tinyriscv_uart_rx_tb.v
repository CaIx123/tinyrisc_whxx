...
module tinyriscv_uart_rx_tb;
    ...
    localparam integer SIM_TIMEOUT_NS = 8000000;
    localparam integer EXPECTED_BYTES = 11;
    ...
    task automatic send_uart_byte(input [7:0] tx_byte);
        ...
        begin
            ...
            uart_rx_pin = 1'b1;
            wait_uart_cycles(UART_BIT_CYCLES * 40);
        end
    endtask
    ...
    initial begin : uart_stimulus
        @(negedge rst_n);
        @(posedge rst_n);
        wait_uart_cycles(UART_BIT_CYCLES * 200);
        send_uart_string();
    end
    ...
    initial begin : uart_monitor_done
    ...
    initial begin
        $readmemh("inst.data", tinyriscv_uart_rx_tb_0.u_exmem_top.u_exrom._ram);
    end

    tinyriscv_sys_top tinyriscv_uart_rx_tb_0(
        .clk(clk),
        .rst(rst_n),
