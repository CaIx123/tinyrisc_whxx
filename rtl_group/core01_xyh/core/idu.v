...
// 纯组合逻辑电路，负责识别指令种类
// 产生立即数、寄存器访问地址、统一的dec_info_bus译码信息总线
// 译码RV32I基础指令 + M扩展乘除指令 + 自定义扩展指令(sendid/readtemp/intfire)
// dec_info_bus是EXU分发的核心，包含ALU/BJP/MULDIV/CSR/MEM/SYS/EXT各组信息
module idu_xyh(
    ...
    // from gpr_reg_xyh
    input wire[31:0] rs1_rdata_i,            // rs1寄存器读数据
    input wire[31:0] rs2_rdata_i,            // rs2寄存器读数据

    output wire stall_o,                     // 译码暂停(分支指令需要)
    ...
    output wire[4:0] rs2_raddr_o,            // rs2寄存器地址
    output wire[31:0] rs1_rdata_o,           // rs1寄存器数据(直通)
    output wire[31:0] rs2_rdata_o,           // rs2寄存器数据(直通)
    output wire[4:0] rd_waddr_o,             // 目标寄存器地址
    output wire rd_we_o                      // 寄存器写使能
    ...
    wire inst_sel_i_imm = inst_addi | inst_slti | inst_sltiu | inst_xori | inst_ori | inst_andi | inst_type_load | inst_jalr | inst_intfire;
